...
final class LogfireTracingManager {
        ...
        try await withCheckedThrowingContinuation { continuation in
            workerQueue.async {
                do {
                    ...
                    NSLog(
                        "[LogfireTracingManager] initialize completed (isInitialized=%@)",
                        self.isInitialized.description)
                    continuation.resume()
                } catch {
    ...
    func recordEvent(
        ...
        workerQueue.async {
            ...
            let resolvedInstrumentationName =
                tracerName.isEmpty ? Constants.defaultTracerName : tracerName
    ...
    private func initializeIfNeeded(serviceName: String, apiKey: String) throws {
        ...
        isInitialized = true
        NSLog(
            "[LogfireTracingManager] tracing initialized for service=%@ apiKey=%@",
            trimmedServiceName, Self.redactedApiKeyDescription(trimmedApiKey))
    }
