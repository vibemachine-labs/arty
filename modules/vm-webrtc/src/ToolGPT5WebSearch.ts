...
export class ToolGPT5WebSearch {
  ...
  constructor(nativeModule: GPT5WebSearchNativeModule | null) {
    ...
    if (this.module) {
      log.info(
        `[${this.toolName}] Initializing; attaching listener for ${this.requestEventName}`,
        {},
      );
      ...
      log.info(
        `[${this.toolName}] Native module unavailable; web search tool disabled.`,
        {},
      );
    }
  }
  ...
  private async handleRequest(event: { requestId: string; query?: string }) {
    ...
    log.info(
      `[${this.toolName}] 📥 Received request from Swift: requestId=${requestId}, queryChars=${query.length}`,
      {},
    );
    ...
    try {
      ...
      log.info(
        `[${this.toolName}] 📡 Delivering result to native bridge`,
        {},
        {
          requestId,
  ...
  private async performOperation(params: GPT5WebSearchParams): Promise<string> {
    ...
    log.info(
      `[${this.toolName}] 🔍 performOperation invoked`,
      {},
      ...
      input: [
        {
          ...
          content: [
            {
              type: "input_text",
              text: "You are a focused research assistant. Use live web search to answer succinctly with citations when available.",
            },
          ],
        },
        {
    ...
    log.info(
      `[${this.toolName}] 📤 Sending payload to OpenAI`,
      {},
      {
        model: payload.model,
    ...
    try {
      ...
      log.info(
        `[${this.toolName}] ❌ Network error calling OpenAI Responses API`,
        {},
        networkError,
      );
    ...
    log.info(
      `[${this.toolName}] 📥 OpenAI response received`,
      {},
      {
    ...
    if (!response.ok) {
      throw new Error(
        `OpenAI Responses API error ${response.status}: ${rawText.slice(0, 400)}`,
      );
    }
    ...
    log.info(
      `[${this.toolName}] 📦 Prepared JS payload for native`,
      {},
      {
    ...
    log.info(
      `[${this.toolName}] ✅ Returning web search payload`,
      {},
      {
        payloadLength: payloadToReturn.length,
        answerPreview: answer.slice(0, 200),
...
export const createGPT5WebSearchTool = (
  ...
  if (!nativeModule) {
    log.info(
      "[ToolGPT5WebSearch] Native module not available. Web search tool will not be initialized.",
      {},
    );
    return null;
  }
