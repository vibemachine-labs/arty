...
export class MCPClient {
  ...
  private async initialize(): Promise<void> {
    ...
    log.info(
      "[MCPClient] Initializing MCP session",
      {},
      {
    ...
    try {
      ...
      if (sessionId) {
        ...
        log.info(
          "[MCPClient] Session established for mcp server",
          {},
          {
            endpoint: this.endpoint,
        ...
        log.warn(
          "[MCPClient] No session ID returned by server",
          {},
          {
            endpoint: this.endpoint,
          },
      ...
      const rawResponse = contentType.includes("text/event-stream")
      ...
      log.info(
        "[MCPClient] MCP session initialized successfully",
        {},
        {
          endpoint: this.endpoint,
          sessionId: this.sessionId,
  ...
  private parseSSEResponse(sseText: string): any {
      ...
      if (line.startsWith("event:")) {
        ...
        if (currentEvent === "message" && currentData) {
          try {
            ...
            log.error(
              "[MCPClient] Failed to parse SSE message data as JSON",
              {},
              {
                data: currentData,
                error: error instanceof Error ? error.message : String(error),
    ...
    if (currentEvent === "message" && currentData) {
      try {
        ...
        log.error(
          "[MCPClient] Failed to parse final SSE message data",
          {},
          {
            data: currentData,
            error: error instanceof Error ? error.message : String(error),
    ...
    log.error(
      "[MCPClient] No valid message event found in SSE response",
      {},
      {
        sseText,
      },
  ...
  private ensureJsonRpcObject(value: unknown): JSONRPCResponse | JSONRPCError {
    if (typeof value !== "object" || value === null) {
      log.error(
        "[MCPClient] Invalid MCP response payload",
        {},
        {
          payload: value,
        },
  ...
  private async request<TParams extends RequestParams | undefined, TResult>(
    ...
    log.info(
      "[MCPClient] Sending JSON-RPC request",
      {},
      {
        endpoint: this.endpoint,
        method: req.method,
    ...
    try {
      ...
      if (!res.ok) {
        log.error(
          "[MCPClient] Network error from MCP server",
          {},
          {
            endpoint: this.endpoint,
            status: res.status,
      ...
      if ("error" in responsePayload) {
        ...
        log.error(
          "[MCPClient] MCP server returned error",
          {},
          {
            endpoint: this.endpoint,
      ...
      if (response.result === undefined) {
        log.error(
          "[MCPClient] Response missing result field",
          {},
          {
            endpoint: this.endpoint,
      ...
      if (!parsed.success) {
        log.error(
          "[MCPClient] Response validation failed",
          {},
          {
            endpoint: this.endpoint,
      ...
      log.info(
        "[MCPClient] JSON-RPC request successful",
        {},
        {
          endpoint: this.endpoint,
          method: req.method,
  ...
  async listTools(
    ...
    log.info(
      "[MCPClient] Listing tools from MCP server",
      {},
      {
        endpoint: this.endpoint,
      },
    ...
    const result = await this.request<
      ...
      ListToolsResult
    >({ method: "tools/list", params }, ListToolsResultSchema, options);

    log.info(
      "[MCPClient] Retrieved tools from MCP server",
      {},
      {
        endpoint: this.endpoint,
  ...
  private validateArgumentsForEndpoint(
    ...
    if (this.endpoint === "https://mcp.deepwiki.com/mcp") {
      ...
      if (toolName === "ask_question") {
        ...
        if (!repoName) {
          throw new Error(
            "deepwiki ask_question requires non-empty repoName argument",
          );
        }

        if (!question) {
          throw new Error(
            "deepwiki ask_question requires non-empty question argument",
          );
        }
      } else if (
        ...
        if (!repoName) {
          throw new Error(
            `deepwiki ${toolName} requires non-empty repoName argument`,
          );
        }
      }
    }
  ...
  async callTool(
    ...
    try {
      ...
      log.error(
        "[MCPClient] Argument validation failed",
        {},
        {
          endpoint: this.endpoint,
    ...
    if (resultLength > MAX_MCP_RESULT_LENGTH) {
      log.warn(
        "[MCPClient] MCP result exceeds max length, truncating",
        {},
        {
          endpoint: this.endpoint,
          toolGroup: toolGroup || "unknown",
    ...
    log.info(
      ...
      {},
      {
        ...
        resultLength:
          resultLength > MAX_MCP_RESULT_LENGTH
            ? MAX_MCP_RESULT_LENGTH
            : resultLength,
        result: result,
      },
    );
